...
#ifdef __AMIGA__
#define dmulscale30r dmulscale30
#define mulscale16r mulscale16
#endif
...
#ifdef __AMIGA__
    hRes = sndLookupRawCached(soundId, pEffect->rawName);
#else
...
{//tstart = getusecticks();
    ...
    earVR.dy = earR.y - earR0.y;//t1 = getusecticks()-tstart;tstart = getusecticks();t2 = t3 = t4 = t5 = t6= 0;
    for (int i = nBonkles - 1; i >= 0; i--)
    {
        BONKLE *pBonkle = BonkleCache[i];
        if (pBonkle->at0 > 0 || pBonkle->at4 > 0)
        {
            ...
            {//tstart = getusecticks();
                ...
                pBonkle->at38 = pBonkle->at10->sectnum;//t6 += getusecticks()-tstart;tstart = getusecticks();
            }//tstart = getusecticks();
            Calc3DValues(pBonkle);//t2 += getusecticks()-tstart;
            MV_Lock();
            if (pBonkle->at0 > 0)
            {
                if (pBonkle->at4 > 0)
                {//tstart = getusecticks();
                    FX_SetPan(pBonkle->at0, lVol, lVol, 0);//t3 += getusecticks()-tstart;tstart = getusecticks();
                    FX_SetFrequency(pBonkle->at0, lPitch);//t4 += getusecticks()-tstart;
                }
                else //{tstart = getusecticks();
                    FX_SetPan(pBonkle->at0, lVol, lVol, rVol);//t3 += getusecticks()-tstart;}
            }
            if (pBonkle->at4 > 0)
            {//tstart = getusecticks();
                FX_SetPan(pBonkle->at4, rVol, 0, rVol);//t3 += getusecticks()-tstart;tstart = getusecticks();
                FX_SetFrequency(pBonkle->at4, rPitch);//t4 += getusecticks()-tstart;
            }
            MV_Unlock();
        }
        else
        {//tstart = getusecticks();
            ...
            pBonkle->at8 = NULL;
            BonkleCache[i] = BonkleCache[--nBonkles];
            BonkleCache[nBonkles] = pBonkle;//t5 += getusecticks()-tstart;
        }
    }
}
