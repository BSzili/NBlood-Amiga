...
static int smk_read_file(void * buf, const size_t size, FILE * fp)
{
...
static int smk_read_memory(void * buf, const unsigned long size, unsigned char ** p, unsigned long * p_size)
{
	if (size > *p_size) {
...
#ifdef __AMIGA__
		if (s->chunk_size[temp_u] > max_chunk_size)
			max_chunk_size = s->chunk_size[temp_u];
#endif
	}

#ifdef __AMIGA__
	smk_malloc(s->video.buffer, max_chunk_size);
#endif
...
int smk_info_all(const smk object, unsigned long * frame, unsigned long * frame_count, double * usf)
{
	/* null check */
	if (object == NULL) {
...
int smk_info_video(const smk object, unsigned long * w, unsigned long * h, unsigned char * y_scale_mode)
{
	/* null check */
	if (object == NULL) {
...
int smk_info_audio(const smk object, unsigned char * track_mask, unsigned char channels[7], unsigned char bitdepth[7], unsigned long audio_rate[7])
{
	unsigned char i;
...
int smk_enable_all(smk object, const unsigned char mask)
{
	unsigned char i;
...
int smk_enable_video(smk object, const unsigned char enable)
{
	/* null check */
	if (object == NULL) {
...
int smk_enable_audio(smk object, const unsigned char track, const unsigned char enable)
{
	/* null check */
	if (object == NULL) {
...
static int smk_render_palette(struct smk_video_t * s, unsigned char * p, unsigned long size)
{
	/* Index into palette */
	unsigned short i = 0;
	/* Helper variables */
...
static int smk_render_video(struct smk_video_t * s, unsigned char * p, unsigned int size)
{
	...
	while (row < s->h) {
		...
		for (j = 0; (j < sizetable[blocklen]) && (row < s->h); j ++) {
...
#ifdef __AMIGA__
				// TODO force the compiler to unroll
				#define LOOPI(i) \
					do { \
						if (unpack & temp) \
							t[skip + (i)] = s1; \
			...
			case 1: /* FULL BLOCK */
				...
				#define LOOP \
					do { \
						unpack = smk_huff16_lookup(&s->tree[SMK_TREE_FULL], &bs); \
...
static int smk_render_audio(struct smk_audio_t * s, unsigned char * p, unsigned long size)
{
	unsigned int j, k;
...
#ifndef __AMIGA__ // TODO this probably breaks seeking and the enable flags
...
#ifdef __AMIGA__
		if (fseek(s->source.file.fp, s->source.file.chunk_offset[s->cur_frame], SEEK_SET)) {
			fprintf(stderr, "libsmacker::smk_first(s) - ERROR: fseek to frame %lu (offset %lu) failed.\n", s->cur_frame, s->source.file.chunk_offset[s->cur_frame]);
			perror("\tError reported was");
			return -1;
		}
#endif
...
int smk_seek_keyframe(smk s, unsigned long f)
{
	/* null check */
	if (s == NULL) {
